...
class notes_documents_support(models.Model):
    _inherit = 'notes.documents.support'
...
class sending_notes_document_support_detail(models.Model):
    _inherit = 'sending.notes.document.support.detail'

    def download_xml(self):
        action = {
            'name': 'XML Documento Soporte',
            'type': 'ir.actions.act_url',
            'url': "web/content/?model=sending.notes.document.support.detail&id=" + str(
    ...
    def get_tax_type(self, percent, get_code):
        ...
        obj_tax = self.env['account.tax'].search([('amount', '=', tmp_percent), ('tax_type.code', '!=', None),
                                                  ('company_id', 'in', self.env.user.company_ids.ids)], limit=1)
    ...
    def get_TAC_TCR(self, tag):
        ...
        if tag == 'TAC':
            for obj in self.move_id.company_id.partner_id.x_tax_responsibilities.filtered(
                    lambda x: x.valid_for_fe == True):
                if to_return:
    ...
    def get_info_ref_Factura(self, field_name):
        ...
        try:
            invoice_name = self.move_id.ref[
                           14: 50 if self.move_id.ref[14:].find(' ') == -1 else self.move_id.ref[14:].find(
                               ' ') if self.move_id.ref.find(',') == -1 else self.move_id.ref.find(',')]
        ...
        if code_position > -1:
            ...
            if status_code >= 400:
                ...
                if method == 'CHECK_FE':
                    ...
                    self.write({'result_status': error_msg})
                if method != 'GET_PDF':
            ...
            if status_code in (200, 201, 202, 203):
                ...
                if method == 'SEND_FE':
                    if '<transaccionID xsi:type="xsd:string">' in obj_result:
                        ...
                        if tmp_transaction_id == '':
                            if '<error xsi:type="xsd:string">El comprobante  ya ha sido firmado previamente. TrasactionID ' in obj_result:
                                tmp_transaction_id = obj_result[obj_result.find(
                                    '<error xsi:type="xsd:string">El comprobante  ya ha sido firmado previamente. TrasactionID ') + len(
                                    '<error xsi:type="xsd:string">El comprobante  ya ha sido firmado previamente. TrasactionID '):obj_result.find(
                                    '</error>')]
                            if tmp_transaction_id == '':
                                self.write({'status': 'ERROR AL ENVIAR XML'})
                            else:
                    ...
                    if '<documentBase64 xsi:type="xsd:string">' in obj_result:
                        ...
                        if error_msg:
                            ...
                            result = self.env['ir.attachment'].create({
                                ...
                                'type': 'binary',
                                'datas': error_msg,
                                'res_model': 'sending.notes.document.support.detail',
                                'res_id': self.id
                            })
    ...
    def send_xml_FE(self):
        ...
        if not base64_file:
            raise UserError(
                _('No se ha generado el archivo a enviar para la facturación electrónica. Por favor verifique!'))
        else:
        ...
        if self.result_status != 'ENVIADO':
            ...
            if not obj_ws:
                raise ValidationError(
                    _("Error! No ha configurado un web service con el nombre 'ftech_ds_status_document'"))
    ...
    def consume_web_service_download_files(self):
        ...
        if not obj_ws:
            raise ValidationError(
                _("Error! No ha configurado un web service con el nombre 'ftech_ds_download_file_document'"))
    ...
    def update_data_move(self):
        ...
        group_moves_df = moves_df.groupby(by=['partner_id', 'move_id', 'first_concept'], group_keys=False,
                                          as_index=False).sum()
