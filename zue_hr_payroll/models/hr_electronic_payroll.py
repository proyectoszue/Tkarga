...
class hr_electronic_payroll_detail(models.Model):
        ...
        try:
            ...
            if operator == 'FacturaTech':
                ...
                if result.find('<code xsi:type="xsd:string">') > -1:
                    result_status = 'Code: '+result[result.find('<code xsi:type="xsd:string">') + len('<code xsi:type="xsd:string">'):result.find('</code>')] +'\n'
        ...
        if operator == 'Carvajal':
            ...
            if self.resource_type_document == 'PDF':
                ...
                if result.find('<documentBase64 xsi:type="xsd:string">') > -1:
                    download_data = result[result.find('<documentBase64 xsi:type="xsd:string">') + len('<documentBase64 xsi:type="xsd:string">'):result.find('</documentBase64>')]
    ...
    def consume_web_service_get_cune(self):
        try:
            ...
            if operator == 'Carvajal':
                ...
                result = obj_ws.connection_requests(username, password, self.electronic_payroll_id.prefix, self.item, not_show_errors=1)
    ...
    def get_value_total_dev(self):
        ...
        for payslip in self.payslip_ids:
            for line in payslip.line_ids:
                value += abs(line.total) if line.salary_rule_id.dev_or_ded == 'devengo' and line.salary_rule_id.code not in lst_codes else 0
        return value
    ...
    def get_value_total_ded(self):
        ...
        for payslip in self.payslip_ids:
            for line in payslip.line_ids:
                value += abs(line.total) if line.salary_rule_id.dev_or_ded == 'deduccion' and line.salary_rule_id.code not in lst_codes else 0
        return value
...
class hr_electronic_payroll(models.Model):
    ...
    z_executing_accepted_ids = fields.One2many('hr.electronic.payroll.detail', 'electronic_payroll_id', string='Ejecución aceptados', domain=[('status', '=', 'ACCEPTED')])
    z_executing_failed_ids = fields.One2many('hr.electronic.payroll.detail', 'electronic_payroll_id', string='Ejecución fallidos', domain=['!', ('status', '=', 'ACCEPTED')])
