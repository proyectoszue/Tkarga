...
class hr_version(models.Model):
    ...
    def get_accumulated_vacation_days(self,ignore_payslip_id=0,method_old=0):
        ...
        if method_old != 0:
            ...
            if ignore_payslip_id == 0:
                days_paid = sum([i.business_units+i.units_of_money for i in self.env['hr.vacation'].search([('version_id', '=', self.id)])])
            else:
                days_paid = sum([i.business_units + i.units_of_money for i in
                                 self.env['hr.vacation'].search([('version_id', '=', self.id),('payslip','!=',ignore_payslip_id)])])
            ...
            if ignore_payslip_id == 0:
                obj_vacation = self.env['hr.vacation'].search([('employee_id', '=', employee_id), ('version_id', '=', self.id)])
            else:
                obj_vacation = self.env['hr.vacation'].search([('employee_id', '=', employee_id), ('version_id', '=', self.id),('payslip','!=',ignore_payslip_id)])
            if obj_vacation:
            ...
            if not date_vacation:
                raise ValidationError('El empleado no tiene fecha de inicio, verificar el contrato actual!')
    ...
    def get_info_book_cesantias(self):
        return self.env['hr.history.cesantias'].search([('version_id','=',self.id)])