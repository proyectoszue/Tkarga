...
class hr_voucher_sending(models.Model):
    ...
    @api.depends('payslip_id', 'payslip_run_id')
    def _compute_display_name(self):
        for record in self:
            if record.payslip_run_id:
                record.display_name = "Comprobantes de nómina - ".format(record.payslip_run_id.name)
            else:
                record.display_name = "Comprobantes de nómina - ".format(record.payslip_id.name)
    ...
    def create_document_inmemory(self,records):
        ...
        for payslip in records:
            ...
            try:
                ...
                pdf_content, content_type = report._render_qweb_pdf(obj_payslip.struct_id.report_id.get_external_id()[obj_payslip.struct_id.report_id.id],[obj_payslip.id])
