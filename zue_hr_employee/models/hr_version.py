...
class hr_contract_change_wage(models.Model):
    ...
    version_id = fields.Many2one('hr.version', 'Contrato', required=True, ondelete='cascade')
    job_id = fields.Many2one('hr.job', string='Cargo')
    rel_employee_id = fields.Many2one('hr.employee', string='Rel Empleado', store=True)
    rel_contract_date_start = fields.Date(string='Rel fecha de inicio', store=True)

    _change_wage_uniq = models.Constraint('unique(version_id, date_start, wage, job_id)', 'Ya existe un cambio de salario igual a este')
    ...
    def create(self, vals_list):
        ...
        def res_id_from_many2one(many2one_value):
            if not many2one_value:
                return False
            if isinstance(many2one_value, int):
                return many2one_value
            return many2one_value.id

        self.env.flush_all()

        keys_in_this_batch = set()
        vals_ready_to_insert = []
        for line_vals in vals_list:
            contract_id = res_id_from_many2one(line_vals.get('version_id'))
            start_date = line_vals.get('date_start')
            wage = line_vals.get('wage')
            job_id = res_id_from_many2one(line_vals.get('job_id')) or False

            batch_key = (contract_id, start_date, wage, job_id)
            if batch_key in keys_in_this_batch:
                continue
            keys_in_this_batch.add(batch_key)

            if self.search_count([
                ('version_id', '=', contract_id), ('date_start', '=', start_date),
                ('wage', '=', wage), ('job_id', '=', job_id)
            ]):
                continue
            vals_ready_to_insert.append(line_vals)

        if not vals_ready_to_insert:
            return self.browse()
        return super().create(vals_ready_to_insert)
...
class hr_contract_concepts(models.Model):
    ...
    version_id = fields.Many2one('hr.version', 'Contrato', required=True, ondelete='cascade')
    type_employee = fields.Many2one('hr.types.employee',string='Tipo de Empleado', store=True)
    z_employee_id = fields.Many2one('hr.employee', string='Empleado',store=True) # VOLVERLO RELATED DESPUES DE LA MIGRACION A V19
    z_branch_id = fields.Many2one(related='z_employee_id.branch_id', store=True, string="Sucursal")
    z_company_id = fields.Many2one(related='z_employee_id.company_id', store=True, string='Compañía')
...
class hr_contract_deductions_rtf(models.Model):
    ...
    # _change_deductionsrtf_uniq = models.Constraint('unique(input_id, version_id)', 'Ya existe esta deducción para este contrato, por favor verficar.')
...
class hr_type_of_jurisdiction(models.Model):
    ...
    _type_of_jurisdiction_uniq = models.Constraint('unique(name)', 'Ya existe este tipo de fuero, por favor verificar.')
...
class hr_version(models.Model):
    ...
    z_state_finished = fields.Boolean(string='Finalizado Por Liquidar')
    company_currency_id = fields.Many2one(related='company_id.currency_id', string='Moneda compañía', store=True)
    wage = fields.Monetary(compute='change_wage', store=True, readonly=False, currency_field='company_currency_id')
    job_id = fields.Many2one('hr.job', string='Puesto de trabajo', compute='change_wage', store=True, readonly=False, tracking=True)
    ...
    change_wage_ids = fields.One2many('hr.contract.change.wage', 'version_id', 'Cambios salario', copy=True)
    concepts_ids = fields.One2many('hr.contract.concepts', 'version_id', 'Devengos & Deducciones', copy=True)
    contract_modification_history = fields.One2many('hr.contractual.modifications', 'version_id','Modificaciones contractuales', copy=True)
    deductions_rtf_ids = fields.One2many('hr.contract.deductions.rtf', 'version_id', 'Deducciones retención en la fuente', default=_get_default_deductions_rtf_ids, tracking=True, copy=True)
    ...
    type_employee = fields.Many2one(string='Tipo de empleado', store=True, related='employee_id.type_employee')
    ...
    #branch_id = fields.Many2one('zue.res.branch', string='Sucursal', related='analytic_account_id.branch_id')
    ...
    schedule_pay = fields.Selection(string='Pago programado', store=True, readonly=True,
                                    help='Se completa automáticamente según la Frecuencia de Pago')
    #Pestaña Fuero
    ...
    @api.model_create_multi
    def create(self, vals_list):
        # Con change_wage_ids en el mismo create, wage y job_id duplican líneas (restricción en hr.contract.change.wage).
        for contract_vals in vals_list:
            has_embedded_salary_changes = bool(contract_vals.get('change_wage_ids'))
            if not has_embedded_salary_changes:
                continue
            contract_vals.pop('wage', None)
            contract_vals.pop('job_id', None)
        return super().create(vals_list)

    @api.onchange('method_schedule_pay')
    def _onchange_method_schedule_pay(self):
        for contract in self:
            if contract.method_schedule_pay:
                contract.schedule_pay = contract.method_schedule_pay

    @api.depends('change_wage_ids', 'change_wage_ids.date_start', 'change_wage_ids.wage', 'change_wage_ids.job_id')
    def change_wage(self):
        for record in self:
            if record.change_wage_ids:
                last_change = max(record.change_wage_ids, key=lambda x: x.date_start or date.min)
                record.wage = last_change.wage
                record.job_id = last_change.job_id
                if record.employee_id:
                    emp_vals = {'job_id': last_change.job_id.id if last_change.job_id else False}
                    if last_change.job_id and hasattr(record.employee_id, 'job_title'):
                        emp_vals['job_title'] = last_change.job_id.name
                    record.employee_id.write(emp_vals)
            else:
                record.wage = 0.0
                record.job_id = False
    ...
    def generate_labor_certificate(self):
        ...
        ctx.update({'default_version_id': self.id, 'default_date_generation': fields.Date.today()})
    ...
    def get_average_concept_heyrec(self): #Promedio horas extra
        ...
        slips_ids = model_payslip.search([('date_from','>=',date_start_str),('date_to','<=',today_str),('version_id','=',self.id),('state','=','validated')])
    ...
    def get_average_concept_certificate(self,salary_rule_id,last,average,z_value_contract,z_payment_frequency): #Promedio horas extra
        ...
        if average == True:
            ...
            today_str = today.strftime("%Y-%m-01")
            date_start_str = date_start.strftime("%Y-%m-01")
            slips_ids = model_payslip.search([('date_from','>=',date_start_str),('date_to','<=',today_str),('version_id','=',self.id),('state','in',['validated','paid'])])
            lines_ids = model_payslip_line.search([('slip_id','in',slips_ids.ids),('salary_rule_id','=',salary_rule_id.id)])
            if lines_ids:
                total = sum([i.total for i in model_payslip_line.browse(lines_ids.ids)])
    ...
    def get_grouped_non_salary_benefits(self, certificate_template_detail_ids):
        """
        Calcula el total agrupado de reglas marcadas con z_group_non_salary = True
        """
        total = 0

        # Filtrar solo las reglas que tienen el check de agrupación marcado
        rules_to_group = certificate_template_detail_ids.filtered(
            lambda x: x.z_group_non_salary == True
        )

        if not rules_to_group:
            return 0

        # Sumar los valores de cada regla marcada según su configuración
        for rule_detail in rules_to_group:
            rule_value = self.get_average_concept_certificate(
                rule_detail.rule_salary_id,
    ...
    def generate_report_severance(self):
        ctx = self.env.context.copy()
        ctx.update({'default_z_version_id': self.id})
    ...
    def print_book_vacaciones(self):
        self.ensure_one()
        report = self.env.ref('zue_hr_payroll.report_book_vacation_action')
        return report.report_action(self)

    def print_book_cesantias(self):
        self.ensure_one()
        report = self.env.ref('zue_hr_payroll.report_book_cesantias_action')
        return report.report_action(self)
...
class hr_labor_certificate_history(models.Model):
    ...
    _order = 'version_id,date_generation'
    ...
    _labor_certificate_history_uniq = models.Constraint('unique(version_id, sequence)','Ya existe un certificado con esta secuencia, por favor verificar.')

    @api.depends('sequence', 'version_id')
    def _compute_display_name(self):
        for record in self:
            record.display_name = "Certificado {} de {}".format(record.sequence,record.version_id.display_name)

    def get_hr_labor_certificate_template(self):
        obj = self.env['hr.labor.certificate.template'].search([('company_id','=',self.version_id.employee_id.company_id.id)])
        if len(obj) == 0:
    ...
    def create(self, values_list):
        ...
        obj_contract = super(hr_labor_certificate_history, self).create(values_list)
        return obj_contract

    def generate_report(self):
        datas = {
            'ids': self.version_id.ids,
        ...
        # V16
        pdf = self.env['ir.actions.report'].sudo()._render_qweb_pdf('zue_hr_employee.report_certificacion_laboral_action', [self.id])[0]

        pdf = base64.b64encode(pdf)
        self.pdf = pdf#base64.encodebytes(pdf)
        self.pdf_name = f'Certificado - {self.version_id.display_name} - {self.sequence}.pdf'

        #Guardar en documentos
        # Crear adjunto
        name = f'Certificado - {self.version_id.display_name} - {self.sequence}.pdf'
        obj_attachment = self.env['ir.attachment'].create({
            ...
            'res_id': self.version_id.employee_id.work_contact_id.id,
            'datas': pdf,
        })
        ...
        if folder_id and self.env.get('documents.document'):
            doc_vals = {
                'name': name,
                'owner_id': self.version_id.employee_id.user_id.id if self.version_id.employee_id.user_id else self.env.user.id,
                'partner_id': self.version_id.employee_id.work_contact_id.id,
                'folder_id': folder_id,
                'tag_ids': tag_ids,
                'type': 'binary',
...
class zue_retirement_severance_pay(models.Model):
    ...
    z_version_id = fields.Many2one('hr.version',string='Contrato')
    ...
    def generate_report_severance_pay(self):
        ...
        pdf = self.env['ir.actions.report'].sudo()._render_qweb_pdf('zue_hr_employee.report_retirement_severance_pay_action',[self.id])[0]
        pdf = base64.b64encode(pdf)
        self.pdf = pdf  # base64.encodebytes(pdf)
        self.pdf_name = f'Carta para retiro de cesantías - {self.z_version_id.name}.pdf'

        # Guardar en documentos
        # Crear adjunto
        name = f'Carta para retiro de cesantías - {self.z_version_id.name}.pdf'
        obj_attachment = self.env['ir.attachment'].create({
            'name': name,
            'store_fname': name,
            'res_name': name,
            'type': 'binary',
            'res_model': 'res.partner',
            'res_id': self.z_version_id.employee_id.work_contact_id.id,
            'datas': pdf,
        })
        # Asociar adjunto a documento de Odoo
        company = self.env.user.company_id
        ...
        if folder_id and self.env.get('documents.document'):
            doc_vals = {
                'name': name,
                'owner_id': self.z_version_id.employee_id.user_id.id if self.z_version_id.employee_id.user_id else self.env.user.id,
                'partner_id': self.z_version_id.employee_id.work_contact_id.id,
                'folder_id': folder_id,
                'tag_ids': tag_ids,
                'type': 'binary',
                'attachment_id': obj_attachment.id
