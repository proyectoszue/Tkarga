...
class zue_custom_alerts(models.Model):
    _name = 'zue.custom.alerts'
    ...
    def create(self, values):
        ...
        return super(zue_custom_alerts, self).create(values)

    def write(self, values):
        if values.get('body_html'):
            values['body_html'] = values['body_arch']
        return super(zue_custom_alerts, self).write(values)
    ...
    def send_alert(self):        
        for record in self:
            ...
            for obj in obj_alert:
                ...
                #emails = list(set(['luis.buitron@zbox.com.co']))
