...
class hr_report_expenses_by_employee(models.Model):
    ...
    departamento = fields.Many2one('hr.department', string='Departamento', readonly=True)
    puesto_trabajo = fields.Many2one('hr.job', string='Puesto de trabajo', readonly=True)
    tipo_contrato = fields.Char(string='Tipo de contrato', readonly=True)
    fecha_finalizacion_contrato = fields.Date(string='Fecha de finalización de contrato', readonly=True)
    tipo_fuero = fields.Many2one('hr.type.of.jurisdiction', string='Tipo de Fuero', readonly=True)

    # SE COMENTA DADO QUE SE DEBE MIGRAR LA CONSULTA - CAMPO VERSION_ID NO EXISTE EN hr_executing_social_security
    @api.model
    def _select(self):
        return f'''
			...
			estructura,liquidacion,estado_de_liquidacion,descripcion,contrato,estado_de_contrato,fecha_liquidacion,fecha_inicial,fecha_final,compania,sucursal,identificacion,empleado,ubicacion_laboral,
                    cuenta_analitica,proyecto,secuencia_contrato,categoria_regla,regla_salarial,secuencia_regla,entidad,unidades,valor,departamento,puesto_trabajo,tipo_contrato,fecha_finalizacion_contrato,tipo_fuero,
                    {SUPERUSER_ID} as create_uid, CURRENT_DATE as create_date, {SUPERUSER_ID} as write_uid, CURRENT_DATE as write_date
            From (
			--NOMINA
            Select upper(a.struct_process) as estructura,a.id as liquidacion,
            case when a."state" = 'draft' then 'Borrador'
                    ...
                    case when e.contract_date_end is null then 'En proceso'
                    else case when e.contract_date_end is not null then 'Expirado'
                        else ''
                        end
                    ...
                    aa.total as valor,e.department_id  as departamento,e.job_id  as puesto_trabajo,e.contract_type as tipo_contrato,e.contract_date_end as fecha_finalizacion_contrato,e.type_of_jurisdiction as tipo_fuero
            ...
            inner join res_partner as d on c.partner_encab_id = d.id
            ...
            UNION ALL
			--ACUMULADOS NOMINA
            Select 'ACUMULADOS' as estructura,0 as liquidacion,'' as estado_de_liquidacion,'Tabla de acumulados' as descripcion,'' as contrato,'' as estado_de_contrato,
			...
			Select 'SEGURIDAD SOCIAL' as estructura,0 as liquidacion,
					...
					case when hc.contract_date_end is null then 'En proceso'
                    else case when hc.contract_date_end is not null then 'Expirado'
                        else ''
                        end
                    end as estado_de_contrato,
                    ...
                    case when cch.process = 'ss_empresa_salud' then coalesce(ae."nValorSaludEmpresa",0) + coalesce(ae."nDiferenciaSalud",0)
	                    ...
	                    end as valor,hc.department_id  as departamento,hc.job_id as puesto_trabajo,hc.contract_type as tipo_contrato,hc.contract_date_end as fecha_finalizacion_contrato,hc.type_of_jurisdiction as tipo_fuero
			...
			inner join hr_version as hc on ae.version_id = hc.id
            inner join res_company as c on b.company_id = c.id
			...
			--PROVISIONES
			--PROVISIONES
			Select 'PROVISIONES' as estructura,0 as liquidacion,
			case when a."state" = 'draft' then 'Borrador'
						else case when a."state" = 'done' then 'Realizado'
							else case when a."state" = 'accounting' then 'Contabilizado'
								else ''
                    ...
                    '' as entidad,
                    1 as unidades,
                    coalesce(ep.value_balance ,0) as valor,hc.department_id  as departamento,hc.job_id as puesto_trabajo,hc.contract_type as tipo_contrato,hc.contract_date_end as fecha_finalizacion_contrato,hc.type_of_jurisdiction as tipo_fuero
			...
			inner join hr_version as hc on ep.version_id = hc.id
            inner join res_company as c on b.company_id = c.id
            inner join res_partner as d on b.partner_encab_id = d.id
            left join zue_res_branch as h on b.branch_id = h.id
            left join res_partner as i on b.partner_encab_id = i.id
