...
class account_tax(models.Model):
    ...
    is_base_affected_only_taxes = fields.Boolean(string="Base afectada unicamente por los impuestos anteriores")
    ...
    #------------------------------------------------------------------------------------------------------------------
    #--------------------- SE HEREDA EL METODO ORIGINAL DE ODOO PARA AGREGAR LÓGICA ZUE -------------------------------
    #------------------------------------------------------------------------------------------------------------------
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True, include_caba_tags=False):
        ...
        sum_tax_amount,sum_tax_amount_selected = 0,[] #Variables ZUE
        for tax in taxes:
            price_include = self._context.get('force_price_include', tax.price_include)

            if price_include or tax.is_base_affected:
                if tax.is_base_affected_only_taxes: # Lógica ZUE
                    tax_base_amount = sum_tax_amount
                elif tax.base_affected_only_taxes_id:
                    tax_base_amount = 0
                    for tax_selected in sum_tax_amount_selected:
                        tax_base_amount += tax_selected[1] if tax_selected[0] == tax.base_affected_only_taxes_id.name else 0 # Fin Lógica ZUE
                else:
            ...
            if not skip_checkpoint and price_include and total_included_checkpoints.get(i) and sum_repartition_factor != 0:
                ...
                # if (base + cumulated_tax_included_amount) < tax.minimum_base and tax.has_minimum_base:
                #     tax_amount = 0
                # else:
                tax_amount = total_included_checkpoints[i] - (base + cumulated_tax_included_amount)
                cumulated_tax_included_amount = 0
            else:
                # if tax_base_amount < tax.minimum_base and tax.has_minimum_base:
                #     tax_amount = 0
                # else:
            ...
            sum_tax_amount += tax_amount  # Lógica ZUE
            sum_tax_amount_selected.append((tax.name,tax_amount)) # Lógica ZUE
